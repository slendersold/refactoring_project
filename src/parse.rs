...
fn do_unquote_non_escaped<'a>(input: &'a str) -> Result<(&'a str, String), ()> {
    ...
    if 0 == quote_byteidx || input.as_bytes().get(quote_byteidx.wrapping_sub(1)) == Some(&b'\\') {
        return Err(());
    }
      ...
      A2: Parser,
{
    ...
    fn parse<'a>(&self, input: &'a str) -> Result<(&'a str, Self::Dest), ()> {
        ...
        for order in orders {
            if let Ok(result) = try_permutation3(input, &self.parsers, order) {
                return Ok(result);
            }
        }
        Err(())
...
fn try_permutation3<'a, A0, A1, A2>(
    input: &'a str,
    parsers: &(A0, A1, A2),
    order: [usize; 3],
) -> Result<(&'a str, (A0::Dest, A1::Dest, A2::Dest)), ()>
where
    A0: Parser,
    A1: Parser,
    A2: Parser,
{
    match order {
        [0, 1, 2] => {
            let (r, a0) = parsers.0.parse(input)?;
            let (r, a1) = parsers.1.parse(r)?;
            let (r, a2) = parsers.2.parse(r)?;
            Ok((r, (a0, a1, a2)))
        }
        [0, 2, 1] => {
            let (r, a0) = parsers.0.parse(input)?;
        ...
        [1, 0, 2] => {
            let (r, a1) = parsers.1.parse(input)?;
            let (r, a0) = parsers.0.parse(r)?;
            let (r, a2) = parsers.2.parse(r)?;
            Ok((r, (a0, a1, a2)))
        }
        [1, 2, 0] => {
        ...
        [2, 0, 1] => {
            let (r, a2) = parsers.2.parse(input)?;
            let (r, a0) = parsers.0.parse(r)?;
            let (r, a1) = parsers.1.parse(r)?;
            Ok((r, (a0, a1, a2)))
        }
...
impl<T: Parser> Parser for Take<T> {
    type Dest = Vec<T::Dest>;
    fn parse<'a>(&self, input: &'a str) -> Result<(&'a str, Self::Dest), ()> {
        (0..self.count).try_fold((input, Vec::new()), |(remaining, mut acc), _| {
            let (remaining, item) = self.parser.parse(remaining)?;
            acc.push(item);
            Ok((remaining, acc))
        })
    }
...
pub fn parse_log_line(line: &str) -> Option<LogLine> {
    let (remaining, log) = <LogLine as Parsable>::parser().parse(line).ok()?;
    remaining.trim().is_empty().then_some(log)
}
...
mod test {
    ...
    fn test_do_unquote_non_escaped() {
        assert_eq!(do_unquote_non_escaped(r#""411""#.into()), Ok(("", "411".into())));
    ...
    fn test_delimited() {
        ...
        assert_eq!(delimited(tag("[".into()), stdp::U32, tag("]".into())).parse("[0x32] nice"), Ok((" nice", 0x32)));
        assert_eq!(delimited(tag("[".into()), stdp::U32, tag("]")).parse("0x32]"), Err(()));
        assert_eq!(delimited(tag("[".into()), stdp::U32, tag("]")).parse("[0x32"), Err(()));
    }
    ...
    fn test_asset_dsc() {
        ...
        assert_eq!(AssetDsc::parser().parse(r#" AssetDsc { "id" : "usd" , "dsc" : "USA dollar" , } "#), Ok(("", AssetDsc{id: "usd".into(), dsc: "USA dollar".into()})));
        assert_eq!(AssetDsc::parser().parse(r#" AssetDsc { "id" : "usd" , "dsc" : "USA dollar" , } nice "#), Ok(("nice ".into(), AssetDsc{id: "usd".into(), dsc: "USA dollar".into()})));

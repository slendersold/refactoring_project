pub mod parse;
use parse::{parse_log_line, LogLine, LogKind, AppLogKind, AppLogJournalKind, SystemLogKind};
...
pub fn read_log<R: std::io::Read + std::fmt::Debug>(
    input: R,
    mode: ReadMode,
    request_ids: &[u32],
) -> Vec<LogLine> {
    ...
    std::io::BufReader::new(input)
        ...
        .filter_map(|line| {
            ...
            matches_mode(mode, &log).then_some(log)
        })
        .collect()
}

fn matches_mode(mode: ReadMode, log: &LogLine) -> bool {
    match mode {
        ReadMode::All => true,
...
App::Journal BuyAsset UserBacket{"user_id":"Alice","backet":Backet{"asset_id":"milk","count":5,},} requestid=10
    ...
    fn test_all() {
        assert_eq!(
            read_log(SOURCE1.as_bytes(), ReadMode::All, &[]).len(),
            1
        );
